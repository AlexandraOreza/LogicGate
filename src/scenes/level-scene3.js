...
export class LevelScene3 extends Phaser.Scene {
  ...
  create() {
    ...
    let info, posterNOT, posterAND, posterOR;

    this.#poster.on("pointerup", () => {
      if (!posterNOT && !info) {
        posterNOT = posterLayer.add(
          this.add.image(300, 200, POSTER_ASSET_KEYS.POSTER_NOT).setScale(0.6)
        );
        posterAND = posterLayer.add(
          this.add.image(680, 200, POSTER_ASSET_KEYS.POSTER_AND).setScale(0.5)
        );
